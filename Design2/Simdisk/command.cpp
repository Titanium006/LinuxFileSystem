...
void dir_info(unsigned int InodeNum) {
	...
	cout.width(20);
	//sstream_toshell.width(20);
	cout << left << "目录名/文件名";
	//sstream_toshell << left << "目录名/文件名";
	sstream_toshell << "目录名/文件名";
	cout.width(20);
	//sstream_toshell.width(20);
	cout << left << "修改日期";
	//sstream_toshell << left << "修改日期";
	sstream_toshell << "修改日期";
	cout.width(4);
	//sstream_toshell.width(4);
	cout << "类型";
	sstream_toshell << "类型";
	cout.width(18);
	//sstream_toshell.width(18);
	cout << right << "大小（字节）";
	//sstream_toshell << right << "大小（字节）";
	sstream_toshell << "大小（字节）";
	cout.width(15);
	//sstream_toshell.width(15);
	cout << right << "物理地址";
	//sstream_toshell << right << "物理地址";
	sstream_toshell << "物理地址";
	cout.width(15);
	//sstream_toshell.width(15);
	cout << right << "所有者";
	//sstream_toshell << right << "所有者";
	sstream_toshell << "所有者";
	cout.width(15);
	//sstream_toshell.width(15);
	cout << right << "属性";
	//sstream_toshell << right << "属性";
	sstream_toshell << "属性";
	cout << endl;
	sstream_toshell << '\n';
	...
	if (!fs.is_open()) error(READ_FAILED);
		...
		for (unsigned int i = 0; i < tem_dir.child_num; i++) {
			...
			cout.fill('0');
			//sstream_toshell.fill('0');
			cout.width(4);
			//sstream_toshell.width(4);
			cout << "\t" << right << tmCreatedTime.tm_year + 1900;
			//sstream_toshell << "\t" << right << tmCreatedTime.tm_year + 1900;
			sstream_toshell << "\t" << tmCreatedTime.tm_year + 1900;
			cout << "/";
			sstream_toshell << "/";
			cout.width(2);
			//sstream_toshell.width(2);
...
void cmd_md(const char* strPath, file_attrib privilege) {
	...
	if (analyse_path(strPath, dirTemp, strDirName)) {//路径合法
		...
		else
		{
			...
			dirTemp.child_num++;
			if (dirTemp.Inode_num == cur_dir.Inode_num)	cur_dir = dirTemp;
			//保存
			fs.open("Virtual_Disk.bin", ios::out | ios::binary | ios::_Nocreate);
			if (!fs.is_open()) error(WRITE_FAILED);
			int i;
...
void delete_file(Dir& dirRemove, unsigned int nIndex) {
	...
	dirRemove.child_num--;
	if (cur_dir.Inode_num == dirRemove.Inode_num)cur_dir = dirRemove;
	fs.open("Virtual_Disk.bin", ios::out | ios::binary | ios::_Nocreate);
	if (!fs.is_open()) error(WRITE_FAILED);
	int i;
	for (i = 0; i < BLOCK_GROUPS_NUM; i++) {
...
void remove_dir(Dir& dirTemp, Dir& dirRemove) {
	...
	dirTemp.child_num--;
	if (cur_dir.Inode_num == dirTemp.Inode_num)cur_dir = dirTemp;
	fs.open("Virtual_Disk.bin", ios::out | ios::binary | ios::_Nocreate);
	if (!fs.is_open()) error(WRITE_FAILED);
	int i;
	for (i = 0; i < BLOCK_GROUPS_NUM; i++) {
...
void save_file(char* strFileName, char* strBuffer, unsigned int strBufferSize, Dir dirTemp, file_attrib privilege) {
	...
	if (!bExist) { // 文件不存在，则进行分配空间和创建i节点
		...
		if (Addr < 0) { // 空间不足
			...
			dirTemp.child_num++;
			if (dirTemp.Inode_num == cur_dir.Inode_num)    cur_dir = dirTemp; // 更新当前目录
		}
	}
...
void user_input(char* strBuffer, unsigned long& strBufferSize) {
	...
	//strBuffer[strBufferSize - 2] = '\0';
	//strBuffer[strBufferSize - 3] = '\0';
	//strBuffer[strBufferSize - 4] = '\0';
	//strBuffer[strBufferSize - 5] = '\0';
	//strBuffer[strBufferSize - 6] = '\0';
	//strBuffer[strBufferSize - 7] = '\0';
	//strBuffer[strBufferSize - 8] = '\0';
	//strBuffer[strBufferSize - 9] = '\0';
	//strBuffer[strBufferSize - 10] = '\0';
	//strBuffer[strBufferSize - 11] = '\0';
	//strBuffer[strBufferSize - 12] = '\0';
	//strBuffer[strBufferSize - 13] = '\0';
	//strBuffer[strBufferSize - 14] = '\0';
	//strBuffer[strBufferSize - 15] = '\0';
	cout << "\n请输入文件内容，按$结束" << endl;
	sstream_toshell << "\n请输入文件内容，按$结束" << endl;
	ReleaseSemaphore(mutexSim[1], 1, NULL);
	WaitForSingleObject(mutexSim[0], INFINITE);
