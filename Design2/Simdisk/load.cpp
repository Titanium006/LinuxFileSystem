...
void load() {
	...
	while (!fs.is_open()) {
		cout << "**********************磁盘初始化中**********************" << endl;
		sstream_toshell << "**********************磁盘初始化中**********************\n";
		fs.clear();
		cmd_init();
		cout << "**********************磁盘初始化完成**********************" << endl;
		sstream_toshell << "**********************磁盘初始化完成**********************\n";
		system("cls");
		return;
	}
	...
	fs.close();
	strcpy(cur_dirName, indTable[cur_dir.Inode_num].fileName);
	//获取用户信息
	get_user_info();
}
...
void cmd_init() {
	...
	indTable[0].havingBlocks = DIR_SIZE;

	//数据块位图初始化
	for (int i = 0;i < BLOCKS_NUM;i++)bsBlockBmp[i] = NOT_USED;
	for (int i = 0;i < indTable[0].havingBlocks;i++)bsBlockBmp[i] = USED;
	//i-节点位图初始化
	...
	cur_dir.Inode_num = 0;
	cur_dir.child_num = 2;//分别是. ..
	cur_dir.SubInode_num[0] = 0;//.表示当前文件 ..表示父文件 二者均指向根目录
	cur_dir.SubInode_num[1] = 0;
	...
	sstream_toshell << "\n\n******************************\n";
	cout << "正在初始化用户信息……\n";
	sstream_toshell << "正在初始化用户信息……\n";
	cout << "注册管理员账户中……\n";
	sstream_toshell << "注册管理员账户中……\n";
	register_user();
	get_user_info();
	memset(cur_user_Id, 0, MAX_ID_LENGTH);
	strcpy(cur_user_Id, systemUsers[0].get_user_id());
	cur_user_tType = systemUsers[0].get_user_type();
	//初始化完成，返回
	cout << "\n******************************\n";
	sstream_toshell << "\n******************************\n";
	cout << "初始化完成！正在转入登录界面……\n";
	sstream_toshell << "初始化完成！正在转入登录界面……\n";
	Sleep(800);
	//system("cls");
}
...
void get_user_info()
{
	...
	//打开 usr 目录，读取文件
	for (unsigned int i = 2; i < dirUsr.child_num; i++)
	{
		//遍历文件，读取帐号信息
		if (indTable[dirUsr.SubInode_num[i]].fileType == TYPE_FILE)
		{
			//读取用户的资料
			open_file(dirUsr.SubInode_num[i], strInfo);
			ssStream << strInfo;
			ssStream >> strPwd >> strUserType;
			int nUserType = atoi(strUserType);
			//写入内存用户类
			systemUsers[i - 2].set_user_info(indTable[dirUsr.SubInode_num[i]].fileName, strPwd, user_type(nUserType));
			ssStream.clear();
		}
	}
	cur_Idsum = dirUsr.child_num - 2;
}