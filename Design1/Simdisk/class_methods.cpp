...
//设置用户信息
void User::set_user_info(const char* user_Id, const char* user_Password, user_type user_tType) {
	strcpy_s(this->user_Id, user_Id);
	strcpy_s(this->user_Password, user_Password);
...
//比较判断密码是否正确
bool User::cmp_pwd(const char* user_Password) {
	if (strcmp(this->user_Password, user_Password) == 0)
		return true;
	else return false;
}
