		...
		if (strcmp(systemUsers[i].get_user_id(), strUserId) == 0)
		{
			...
			if (analyse_path(path, dirTemp, strDirName)) {//路径合法
				...
				if (cur_user_tType != ADMIN && cur_user_tType != SYSTEM && indTable[dirTemp.Inode_num].fileAttr != SHARE &&
					strcmp(indTable[dirTemp.Inode_num].user_Id, cur_user_Id) != 0)
				{
				...
				if (!dirTemp.have_child(strDirName, nIndex, TYPE_FILE))
				{
					...
					if (cur_user_tType != ADMIN && cur_user_tType != SYSTEM && indTable[nIndex].fileAttr != SHARE &&
						strcmp(indTable[nIndex].user_Id, cur_user_Id) != 0)
					{
						error(NO_PRIVILEGE, path);
						cout << endl;
						error(DEL_FAILED);
...
void cmd_md(const char* strPath, file_attrib privilege) {
	...
	if (analyse_path(strPath, dirTemp, strDirName)) {//路径合法
		...
		if (cur_user_tType != ADMIN && cur_user_tType != SYSTEM && strcmp(indTable[dirTemp.Inode_num].fileName, "root") != 0 &&
			indTable[dirTemp.Inode_num].fileAttr != SHARE &&
			strcmp(indTable[dirTemp.Inode_num].user_Id, cur_user_Id) != 0 || (strcmp(indTable[dirTemp.SubInode_num[1]].fileName, "root") == 0 && strcmp(indTable[dirTemp.Inode_num].fileName, "user") == 0))
		...
		else
		{
			...
			strcpy(indTable[dirNew.Inode_num].user_Id, cur_user_Id);//配置此目录所属的用户名
...
void remove_dir(Dir& dirTemp, Dir& dirRemove) {
	...
	if (dirRemove.child_num <= 2) {//该目录下无子目录项
		...
		cout << "Tips:" << indTable[dirRemove.Inode_num].fileName << "目录下不为空，是否要继续删除<Y/N> ?";
		char init_choice = '\0';
		while (init_choice != 'y' && init_choice != 'Y' && init_choice != 'n' && init_choice != 'N') {
			init_choice = _getch();
		}
		...
		int dirRemove_son = dirRemove.child_num;
		for (unsigned int i = dirRemove_son - 1; i >= 2; i--) {
			if (indTable[dirRemove.SubInode_num[i]].fileType == TYPE_DIR) {//有子目录项，则递归实现删除
				Dir dirRemoveSon;
				fs.open("Virtual_Disk.bin", ios::in | ios::binary);
				if (!fs.is_open())error(READ_FAILED);
				else {
					fs.seekg(indTable[dirRemove.SubInode_num[i]].BlockAddr, ios::beg);
				...
				delete_file(dirRemove, dirRemove.SubInode_num[i]);
			}
...
void save_file(char* strFileName, char* strBuffer, unsigned int strBufferSize, Dir dirTemp, file_attrib privilege) {
	...
	if (!bExist) { // 文件不存在，则进行分配空间和创建i节点
		...
		if (Addr < 0) { // 空间不足
			...
			strcpy(indTable[nInode].user_Id, cur_user_Id); // 设置用户ID
...
void cmd_del(const char* strPath) {
	...
	if (analyse_path(strPath, dirTemp, strDirName)) {//路径合法
		//判断用户权限
		if (cur_user_tType != ADMIN && cur_user_tType != SYSTEM && indTable[dirTemp.Inode_num].fileAttr != SHARE && strcmp(indTable[dirTemp.Inode_num].fileName, "root") != 0 &&
			strcmp(indTable[dirTemp.Inode_num].user_Id, cur_user_Id) != 0 || (strcmp(indTable[dirTemp.Inode_num].fileName, "user") == 0 && strcmp(indTable[dirTemp.SubInode_num[1]].fileName, "root") == 0))
		{
			error(NO_PRIVILEGE, strPath);
			cout << endl;
			error(DEL_FAILED);
