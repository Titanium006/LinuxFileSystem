...
class User {
private:
	char user_Id[MAX_ID_LENGTH];			//用户账号最长 10 位
	char user_Password[MAX_PWD_LENGTH];	//用户密码 最长 20 位
	user_type user_tType;					//用户

public:

	void set_user_info(const char* user_Id, const char* user_Password, user_type user_tType);//设置用户信息
...
{
public:
	unsigned int Inode_num;						//i-结点号
	unsigned int child_num;						//子文件+子目录总数
	unsigned int SubInode_num[MAX_SUBITEM_NUM];	//子项目i-结点
...
public:
	...
	char		user_Id[MAX_ID_LENGTH];		//文件拥有者
	unsigned long fileSize;					//大小
	unsigned long BlockAddr;				//磁盘块起始地址
	unsigned int havingBlocks;				//占用磁盘块数
};
...
// 保存所有命令，以便于在调用help函数的时候查询相对应的指令然后输出对应的帮助提示
static const char* strCommands[COMMAND_COUNT] =
{
	"info","cd", "dir", "md", "rd", "newfile",
	"cat","write","copy", "del","check","init","cls",
	"help","logon","useradd", "userdel", "exit"
};

//头文件中不允许定义变量，因此定义到simdisk.cpp，再通过extern放入simdisk.h中
extern char cur_user_Id[MAX_ID_LENGTH];				//当前用户账号ID
extern user_type cur_user_tType;						//当前用户类型
